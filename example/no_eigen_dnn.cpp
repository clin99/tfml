...
struct MNIST {
  ...
  struct Node {
    ...
    void forward() {
      ...
      switch(act) {
        case Activation::RELU:  y = std::max(0.0, y); break;
        case Activation::SIGMOID: y = 1.0/(1.0 + std::exp(-y)); break;
      };
    }
  ...
  void softmax(std::vector<double>& values) {
    ...
    double sum {0.0};
    std::for_each(values.begin(), values.end(), [&sum](auto& v){ sum += std::exp(v); }); 
...
int main(){
  ...
  tf::Taskflow tf(4);
  MNIST dnn;
  dnn.add_layer(784, 30, MNIST::Activation::RELU);
  dnn.add_layer(30, 10, MNIST::Activation::NONE);
