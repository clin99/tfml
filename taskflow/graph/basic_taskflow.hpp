...
class BasicTaskflow : public FlowBuilder {
  ...
  private:
    ...
    std::mutex _mtx;
    std::unordered_map<Framework*, TopologyView> _frameworks;
...
//// Function: run
//template <template <typename...> typename E>
//auto BasicTaskflow<E>::run(Framework& framework) const {
//  std::lock_guard<std::mutex> lock(_mtx);
//  if(_frameworks.find(&framework) == _frameworks.end()) {
//    auto [success, iter] = _frameworks.insert(&framework, framework);
//    
//  }
//  else {
//  }
...
void BasicTaskflow<E>::silent_dispatch() {
  ...
  if(_topologies.empty()) {
    ...
    auto& topology = _topologies.front();
    _schedule(topology._sources);
  }
}
